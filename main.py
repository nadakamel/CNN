...
def generate_testing_dataset(num_of_images_to_select):
    for category in FOLDERS_NAMES:
        group_of_items = []
        for filename in glob.iglob(TRAINING_DIR_PATH + category + '/*.jpg'):
        ...
        for path in list_of_random_items:
            shutil.move(path, TESTING_DIR_PATH)
'''
...
def train_model():
    ...
    # compile the model
    model.compile(optimizer=optimizer, loss='categorical_crossentropy', metrics=['accuracy'])   # categorical_crossentropy iss for negative log likelihood
    ...
    model.fit_generator(
            ...
            validation_steps=NUM_OF_TESTING_IMAGES//BATCH_SIZE,
